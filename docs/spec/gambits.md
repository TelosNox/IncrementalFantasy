...
1. **Manuell** – der Spieler wählt (Klicker-Einstieg und spätere Übernahme).
...
Der allererste Abschnitt ist ein **kurzer manueller Klicker**: Aktionen (Angriff, Magie, Limit) werden bei ATB-Bereitschaft selbst gewählt.

- **Zweck:** lehrt das Kampf-Vokabular (Angriff, MP, Magie, Limit, Shock). Man kann keine gute Automatik schreiben für ein System, das man nicht verstanden hat.
...
Ab da ist **Autoplay der Standard**; der Spieler kann **jede Figur einzeln auf Manuell stellen** und ihre Aktionen selbst wählen (Auto/Manuell-Umschalter **je Figur**).
...
- **Zugänglichkeit:** Sobald eine Manuell-Figur bereit ist, **pausiert die gesamte Kampfuhr** während der Auswahl (wie FF7s „Wait"-Modus) – nicht nur die eigene Leiste, sondern **auch Shock-Auf/-Abbau, Gegner-Telegrafs und Statuseffekt-Ticks** stehen still (Details `kampf-analyse-shock.md` §1). So wird ein Shock-Fenster nie durch Nachdenken verbraucht – überlegt statt hektisch.
...
Der Auto/Manuell-Schalter sitzt **pro Figur**. Spontanes Eingreifen im laufenden Auto-Spiel ist unmöglich – der Gambit feuert sofort bei voller Leiste. Deshalb **stellt man eine Figur vorab auf Manuell**: Das Team kämpft automatisch weiter, während **eine Figur gezielt manuell** spielt (z. B. Claude/Barrel/Tofa auf **Auto**, **Arris** auf **Manuell** für den Zauber zur Schwäche). „Alle manuell" ist einfach der Zustand, in dem jede Figur auf Manuell steht. Default nach Freischaltung: Auto.

### Bedien-Flow der manuellen Aktion

1. Manuell-Figur wird **bereit** (ATB voll) → die **gesamte Kampfuhr pausiert** (globale Wait-Pause).
...
4. Aktion feuert → Uhr läuft weiter. **Mehrere gleichzeitig bereite** Manuell-Figuren werden als **Warteschlange** nacheinander abgefragt.

**Aktions-Gruppen im Popup (skaliert mit dem Fortschritt):**

- **Grundaktionen (immer):** Angriff; **Spezial** (Waffen-Signature, MP-Kosten); **Limit** (nur wenn geladen – dann in **bunten Buchstaben**, FF7-Signatur).
- **Verteidigen:** erscheint als **kontextuelles Event**, sobald der **erste Boss sichtbar eine Groß-Attacke auflädt** (Onboarding genau bei Bedarf; in Kapitel 1 spätestens an Vaultron).
- **Magie ▸ (Materia-Kategorie):** erscheint **erst, wenn Materia-Aktionen vorhanden sind** (ab Kap. 2); öffnet als **scroll-/blätterbare Unterliste** → beliebig viele Zauber, ohne dass das Popup wächst.
- **Nicht ausführbare Aktionen** (z. B. Spezial ohne MP) bleiben **sichtbar, aber ausgegraut** (gedämpft + dünne Schrift) – nie verstecken, damit die Optionen lernbar bleiben.
...
**Der entscheidende Schutz der Zugänglichkeit:** „Manuell" heißt bei uns **überlegt, nicht reflexschnell.** Dank ATB-Pause (Wait-Modus) ist die Herausforderung, *Wissen anzuwenden* (Schwäche/Shock/Limit/MP klug einsetzen) – keine Reaktionszeit. So bleibt es on-genre und auch für entspannte/mobile Spieler machbar.
...
- **Aktionen:** Angriff, Materia-Fähigkeit/Magie, Verteidigen, Limit.
...
Manuell-only (Klicker) → Default-Attack-Regel → mehr Gambit-Slots/Bedingungen/Sets über Reunion. Manuelle Steuerung bleibt durchgehend verfügbar.

**Sichtbarkeits-Rollout der Steuer-UI** (nie mehr zeigen, als gerade Sinn ergibt):

- **Klicker-Auftakt** (vor Default-Attack): **kein** Auto/Manuell-Schalter – es gibt nichts zu automatisieren; das Popup erscheint bei jeder Bereitschaft.
- **Ab Default-Attack-Regel** (Region 1): der **Auto/Manuell-Schalter je Figur** erscheint.
- **Verteidigen** ab der ersten telegrafierten **Boss-Aufladung**; die **Magie-Kategorie** ab der ersten **Materia** (Kap. 2).
...
- Genaue Region der Verteidigen-Freischaltung (welcher Boss lädt zuerst sichtbar auf).
