...
- **Ablesbarkeit von Ventil und Gate – eine Frage, zwei Hälften; eine davon beantwortet (Stand 02.08.2026).** Der gemeinsame Bedarf: *Der Spieler muss sehen, dass sich sein Zustand gegenüber der Wand verbessert.* Die Leitplanke für beide Hälften: **rückblickend ja, vorhersagend nein** – eine Erfolgsprognose entwertet das Ausprobieren.
  - ✓ **Vorstoß (Gate): entschieden.** Der **beste bisherige Versuch** wird am verlorenen Gate angezeigt (`spec/ui-layout.md`). Rein rückblickend, erfüllt die Sichtbarkeits-Bedingung der Gate-Regel (§15).
  - ✓ **Rückweg (Ventil): weitgehend geklärt.** Das HP-Signal löst zuerst das **Gasthaus** aus – und das ist korrekt, weil HP/MP über Kämpfe hinweg tragen und ein vollgeheilter Versuch ein *anderer* Kampf ist. Es entsteht eine **Eskalationsleiter: heilen (billig) → zurückgehen und farmen (teuer)**, die der Determinismus selbst begrenzt (ein zweiter vollgeheilter Versuch verläuft identisch). Details: `spec/niederlage-offline.md` §3. Offen bleibt allein der **Übergang von Stufe 1 zu Stufe 2**.
  - ✓ **Sichtbarkeit der EXP-Dämpfung: entschieden.** Zonen, in denen ein Sieg nichts mehr einbringt, sind als **„erschöpft"** markiert – **binär** (zahlt / zahlt nicht), in der Zonenwahl **und** an der aktuell bespielten Zone, plus eine einmalige Meldung im Kipp-Moment (`spec/ui-layout.md`). Ohne das ist der harte Cutoff von einem Fehler nicht zu unterscheiden. Bewusst **keine** Ertragsstufen: Eine Skala macht aus der Zonenwahl eine Ertragsmaximierung statt einer Entscheidung.
