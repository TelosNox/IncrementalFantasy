...
3. **Debug-Reset-Button jetzt hinter `import.meta.env.DEV`** (`GameScreen.svelte`): im `vite build`-Output vollständig wegoptimiert (per Grep im Bundle verifiziert - der Text „Reset save" taucht im Produktions-Build gar nicht mehr auf), im Dev-Server weiterhin sichtbar. Damit ist die in `05_Architektur.md` §6a seit M0 vorgemerkte Gegenprüfung erledigt.
