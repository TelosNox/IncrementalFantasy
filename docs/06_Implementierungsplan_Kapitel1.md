...
**Umgesetzt (02.08.2026):** Alle neun Umfangspunkte + Abnahme in `src/`. `Stage.svelte` verzweigt auf `game.phase === 'inn'` statt einer eigenen Szenen-Komponente (Begründung s. Umsetzungsentscheidung #107): andere Kulisse (`assets/regions/inn_224.png`), keine Gegner-Ebene, Fensterschein in der Regions-Signaturfarbe per `mix-blend-mode: color` (#108), Dimmung als `filter: brightness()` linear über `INN_DEAD_TIME` auf Kulisse+Figuren, HUD-Leisten bewusst ausgenommen (#109), gestaffelte Ankunft „Stehende zuerst, Gefallene zuletzt" über eine reine Verzögerungs-Map, Aufstehen fällt mit `unit.hp <= 0` automatisch aus der bestehenden Kampf-Schwelle (#110), Aufbruch-Blende rein kosmetisch ohne Store-Verzögerung (#111), Wirtszeilen-Pool `content/innLines.ts` (#112). Nebenbei zwei echte Totzeit-Lecks gefunden und behoben, kein Beschluss geändert: `leaveInn()` und der Auto-Exit in `#advanceInn()` prüften `INN_DEAD_TIME` bisher gar nicht (#106) – feinspec §3.8b („waehrend der Totzeit gibt es keinen Ausstieg") war an beiden Stellen im Code nicht umgesetzt. `tests/inn-scene.test.ts` (4 Tests) deckt beide Fixes ab. `npm test` 138/138, `npm run check` 0 Fehler/Warnungen. Im Browser gespielt: freiwilliger Aufenthalt und erzwungener Aufenthalt nach Wipe, beide mit gestaffelter/getragener Ankunft und automatischem Aufbruch bestätigt. Details: `07_Umsetzungsentscheidungen.md` #106–112.
